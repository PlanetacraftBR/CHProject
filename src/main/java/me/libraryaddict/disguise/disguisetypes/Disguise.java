                        ...
                        if (getType() != DisguiseType.EXPERIENCE_ORB || !getEntity().isOnGround()) {
                            ...
                            if (getType() == DisguiseType.WITHER_SKULL && DisguiseConfig.isWitherSkullPacketsEnabled()) {
                                ...
                                if (isSelfDisguiseVisible() && getEntity() instanceof Player) {
                                    ...
                                    try {
                                       Main.getProtocolManager().sendServerPacket((Player) getEntity(),
                            ...
                            try {
                                ...
                                for (Player player : DisguiseUtilities.getPerverts(disguise)) {
                                    ...
                                    if (lookPacket != null && player != getEntity()) {
                                       Main.getProtocolManager().sendServerPacket(player, lookPacket, false);
                                    }
                                   Main.getProtocolManager().sendServerPacket(player, velocityPacket.shallowClone(),
                                            false);
                                }
                            } catch (Exception e) {
                                e.printStackTrace(System.out);
                            ...
                            for (Player player : DisguiseUtilities.getPerverts(disguise)) {
                                if (getEntity() != player) {
                                   Main.getProtocolManager().sendServerPacket(player, packet, false);
                                    ...
                                    try {
                                       Main.getProtocolManager().sendServerPacket((Player) getEntity(), selfPacket,
                                                false);
                                    } catch (InvocationTargetException e) {
                                        e.printStackTrace(System.out);
                                    }
