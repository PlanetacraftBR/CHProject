  ...
  extends MiniPlugin
{
  ...
  public void recharge()
  {
      ...
      for (String ability : rechargeList)
      {
        ...
        if (this.informSet.contains(ability)) {
          ...
          UtilPlayer.message(cur, "§c§lTEMPO §7Você já pode usar §a" + ability + "§7.");
  ...
  public boolean use(Player player, String ability, String abilityFull, long recharge, boolean inform, boolean attachItem)
  {
    ...
    if (Get(player).containsKey(ability))
    {
      if (inform)
      {
    	  UtilPlayer.message(player, "§c§lTEMPO §7Para usar o §a" + ability + "§7." + " Você deve esperar §9" + UtilTime.Convertor(((RechargeData)Get(player).get(ability)).GetRemaining(), 1, UtilTime.TimeUnit.FIT));
