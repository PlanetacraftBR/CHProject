  ...
  extends MiniPlugin
{
  ...
  @EventHandler
  public void ExpireBlocks(Atualizar event)
  {
  ...
  public void Add(Block block, int toID, byte toData, double d)
  {
    if (!Contains(block)) GetBlocks().put(block, new BlockRestoreData(block, toID, toData, d, 0L)); else {
      GetData(block).update(toID, toData, (long) d);
    }
  }
