...
public class ScriptTrait extends Trait {
    ...
    public void loadScript(final String file) {
        File f = new File(JavaPlugin.getPlugin(Main.class).getScriptFolder(), file);
    ...
    public boolean validateFile(String file) {
        File f = new File(JavaPlugin.getPlugin(Main.class).getScriptFolder(), file);
        if (!f.exists() || !f.getParentFile().equals(JavaPlugin.getPlugin(Main.class).getScriptFolder())) {
            return false;
        }
